FactoryGirl.define do
  ...
  CmmsHelper::required_fields.each do |key|
   ...
   factory :cmm_valid_put_request_domain_name, class: Hash do
      domainName { Faker::Internet.domain_name }
      ...
      initialize_with { attributes }
       ...
       factory :cmm_valid_put_request_host_config, class: Hash do
         hostConfig do
           (0..2).map do
               {
                 :DDNSenabled => possible_values[:boolean].sample,
     ...
     factory :cmm_valid_put_request_hostname, class: Hash do
         hostname { Faker::Internet.domain_name }

        initialize_with { attributes }
      end

      factory :cmm_valid_put_request_ip_interfaces, class: Hash do
        ipInterfaces do
          (0..2).map do
            {
            :IPv4assignments => (0..2).map do
